...
describe('LtpClient', () => {
  ...
  it('persists identifiers during handshake_init', async () => {
  ...
  it('reuses stored thread_id on handshake_resume', async () => {
    const storage = new TestStorage();
  ...
  it('falls back to handshake_init when resume is rejected', async () => {
    const storage = new TestStorage();
  ...
  it('sends heartbeat pings and reconnects when pong missing', async () => {
    const storage = new TestStorage();
    MockWebSocket.serverHandler = (socket, data) => {
      const parsed = JSON.parse(data);
      if (parsed.type === 'handshake_init') {
  ...
  it('uses exponential backoff on reconnect attempts', async () => {
    const storage = new TestStorage();
    MockWebSocket.serverHandler = (socket, data) => {
      const parsed = JSON.parse(data);
      if (parsed.type === 'handshake_init') {
