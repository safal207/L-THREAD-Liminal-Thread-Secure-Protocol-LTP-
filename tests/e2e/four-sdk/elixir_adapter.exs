          ...
          receive do
            next ->
              case matcher.(next) do
                ...
                :skip ->
                  raise "unexpected messages while waiting for #{label}: #{inspect([message, next])}"
              end
          after
...
stale =
  build_event.(state, "#{sdk}:stale-timestamp", %{
...
:sys.replace_state(pid, fn current ->
  %{
    current
    | last_sent_hash: LTP.Crypto.hash_envelope(replay_seed),
