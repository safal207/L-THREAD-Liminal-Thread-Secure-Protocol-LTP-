...
# 2b) Check for legacy --input <file> usage (trace inputs must be .jsonl)
...
LEGACY_INPUT_JSON_HITS=$(
  ...
  | grep -v "scripts/ci-guardrail.sh" \
...
if [ -n "$EXISTING_DIRS" ]; then
  ...
  # 3b) Disallow `ltp inspect` command invocations without a subcommand (trace|replay|explain|help)
  # Restrict to likely command lines that include options (e.g., --input).
